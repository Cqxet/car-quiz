...
function toCar(row: Binding): CarChallenge | null {
  ...
  const yearRaw = row.year?.value || row.inception?.value || "";
  const yearMatch = yearRaw.match(/\b((?:19|20)\d{2})\b/) || label.match(/\b((?:19|20)\d{2})\b/);
...
export async function fetchRemoteCars(): Promise<CarChallenge[]> {
  const query = `
SELECT DISTINCT ?item ?itemLabel ?image ?manufacturerLabel ?inception WHERE {
